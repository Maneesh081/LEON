...
| 3 | `syn_count >= 100` AND (`bwd_packets == 0` OR `rst_count == bwd_packets`) | **BLOCK** (SYN flood rule, conf = 1.0) |
...
- **Rule-based SYN flood detection.** 100+ SYNs with either zero responses OR
  all-RST responses is always an attack — no ML needed. This mirrors real
  firewalls: rules for obvious attacks, ML for novel ones. Fires before the
  ML model, so even if the model is confused (e.g. same-IP loopback traffic
  or SYN floods against closed ports), the rule catches it.
